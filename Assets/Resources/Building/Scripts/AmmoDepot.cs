...
public class AmmoDepot : MonoBehaviour, IDamagable, Displayable, IBuilding, Interactable, ISubscribers
{
    ...
    public void Interact(PlayerController player)
    {
        if (inventory.ContainsKey("Ammo"))
        {
            if (inventory["Ammo"] > 0)
            {
                if (!player.livingBeing.inventory.ContainsKey("Ammo"))
                {
                    player.livingBeing.inventory.Add("Ammo", 0);
                }
                player.livingBeing.inventory["Ammo"] += 1;
                inventory["Ammo"] -= 1;
            }
