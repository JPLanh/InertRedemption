...
//            lv_playerController.playerCamera.transform.localRotation = Quaternion.Euler(rotation.x, 0, 0);
    ...
    {
        if (Time.time >= lastUpdate && (lastRot != rotation || lastPos != transform.position))
        {
            lastPos = transform.position;
            lastRot = rotation;
            Dictionary<string, string> payload = StringUtils.getPositionAndRotation(transform.position, rotation);
